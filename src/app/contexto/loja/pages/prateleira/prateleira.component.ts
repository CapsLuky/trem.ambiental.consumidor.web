import { Router } from '@angular/router';
import { Component, OnDestroy, OnInit } from '@angular/core';

import { Subject } from 'rxjs';
import { finalize, takeUntil } from 'rxjs/operators';

import { NzNotificationService } from 'ng-zorro-antd/notification';

import { PedidoItemModel } from '../../models/PedidoItem.model';
import { IProduto } from '../../models/IProduto';
import { TokenService } from '../../../autenticacao/services/token.service';
import { PrateleiraService } from '../../services/prateleira.service';

@Component({
  selector: 'app-prateleira',
  templateUrl: './prateleira.component.html',
  styleUrls: ['./prateleira.component.scss']
})
export class PrateleiraComponent implements OnInit, OnDestroy {

  public pontos = 0;
  public idUsuario = 0;
  public onPontosLoad = true;
  public produtoList!: IProduto[];
  public pedidoItem!: PedidoItemModel;

  placeholder = '';
  fallback ='data:image/png;base64,';


  private _destroySubscribes$ = new Subject<void>();

  constructor(private readonly _prateleiraService: PrateleiraService,
              private readonly _notificao: NzNotificationService,
              private readonly _tokenService: TokenService,
              private readonly _router: Router,) {
                const tokenModel = this._tokenService.buscarTokenModel();
                this.idUsuario = Number(tokenModel.usuarioId)
               }



  ngOnInit(): void {
    this.buscarProdutosDaLoja();
  }

  buscarProdutosDaLoja() {

    this._prateleiraService.buscarProdutosDaLoja()
    .pipe(
      takeUntil(this._destroySubscribes$),
      finalize(() => {

      })
    )
    .subscribe((result) => {
      if (result.success) {
        this.produtoList = result.payload;

        this.produtoList.forEach(element => {
          element.imagem = '../../../../../assets/foto-produtos/' + element.id + '.jpg'
        });

        if (result.messages.length > 0)
              this._notificao.create(result.messages[0].tipo, result.messages[0].titulo, result.messages[0].mensagem);
      } else {
        this._notificao.error('ERRO', result.errors[0].mensagem, {nzDuration: 0});
      }
    });
  }

  onBtnPedir(id: number){

    this.pedidoItem = new PedidoItemModel(this.idUsuario, id, 1);

    this._prateleiraService.enviarItemParaCarrinho(this.pedidoItem)
    .pipe(
      takeUntil(this._destroySubscribes$),
      finalize(() => {

      })
    )
    .subscribe((result) => {
      if (result.success) {
        if (result.messages.length > 0)
              this._notificao.create(result.messages[0].tipo, result.messages[0].titulo, result.messages[0].mensagem);

        if(result.payload) {
          this._router.navigate(['loja/carrinho']);
        }
      } else {
        this._notificao.error('ERRO', result.errors[0].mensagem, {nzDuration: 0});
      }
    });
  }

  ngOnDestroy() {
    this._destroySubscribes$.next();
    this._destroySubscribes$.complete();
  }
}
